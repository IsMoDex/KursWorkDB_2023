        ...
        private void setTypeForm(RequestLevel level)
        {
            ...
            else if (level == RequestLevel.Individual)
            {
                ...
                RequestList = new SortedDictionary<string, string>()
                {
                    ...
                    { "b) Средний возраст и стаж сотрудников по всем садам №4", "SELECT * FROM avg_age_of_employees" },
                    { "b) Средний возраст и стаж сотрудников для каждого сада №5", "SELECT * FROM avf_age_employees_for_each_garden" },
...
Подсчет общего количества растений из каталога которые прожили меньше @Param% от своего срока жизни, их срок жизни, @Param% от его жизни и среднее прожитих лет из выборки
...
Cады, которые имеют год открытия позже, чем средний год открытия всех садов.";
                ...
                case RequestLevel.Technical:
                    TypeMessage = "Справка: Технические запросы";
                    TextMessage = @"Итоговый запрос без условия c итоговыми данными вида: «Всего» №1
Получить общее количество удобрений и их среднюю цену

Итоговый запрос без условия c итоговыми данными вида: «В том числе» №2
Получить Общее количество семейств и видо а так-же средннюю продолжительность жизни всех растений

Итоговый запрос с условием на данные по значению №3
Получить информацию о садах у которых название типа собственности равно

Итоговый запрос с условием на данные по маске №4
Получить всех сотрудников у которых имя начинается на @Param

Итоговый запрос с условием на данные с использованием индекса №5
Получить информацию о растении цена саженца которога >= @Param

Итоговый запрос с условием на данные без использования индекса №6
Получить информацию о растениях срок жизни которых >= @Param

Запрос с использованием объединения №7
Получить все семейства и виды с подвязкой к семейству

Запрос с подзапросом с использованием IN №8
Получить виды и количество растений для каждого вида имеющие семейство из списка 
('Паслёновые', 'Вахтовые', 'Valerianaceae', 'Дымянковые')

Запрос с подзапросом с использованием NOT IN №9
Получить сколько раз ухаживали за растениями которые не соотвествуют семействам из списка 
('Rosaceae', 'Иксонантовые', 'Стилидиевые', 'Вахтовые', 'Ленноовые', 'Сипаруновые', 'Педалиевые', 'Melastomataceae')

Запрос с подзапросом с использованием CASE №10
Получить сколько прожило растение или сколько оно живет если нет даты смерти а так-же город и сад в котором оно находится

Запрос с подзапросом с использованием операциями над итоговыми данными №11
Получить количество если количество растений для кажого сада >= @Param2 и последняя дата полива которых была позже или равна @Param";
                    break;
